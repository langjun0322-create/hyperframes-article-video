...
当前稳定基线使用 Git tag 标记。`main` 分支保持稳定可用，需要固定一个可回退版本时，再创建新的版本 tag。

后续修改建议按这个流程进行：

1. 先确认 `main` 已经同步到最新远端状态。
2. 从 `main` 创建功能分支，默认命名为 `codex/<任务名>`。
3. 在功能分支上完成修改和验证。
4. 验证通过后提交、推送，再合并回 `main`。
5. 合并后的稳定节点如果需要长期保存，再创建新的版本 tag。

不额外维护长期 `develop` 分支；这个仓库以 `main` + 短期功能分支 + 版本 tag 作为轻量发布方式。
